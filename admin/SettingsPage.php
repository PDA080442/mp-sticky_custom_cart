...
final class SettingsPage {
	...
	private static function render_styles_tab( array $s, $opt ) {
		...
		echo '<table class="form-table" role="presentation"><tbody>';
		self::field_number( $opt, 'sticky_cart', 'tristate_dock_inset_right_px', __( 'Горизонтальный отступ колонки от краёв (px)', 'mp-sticky-custom-cart' ), isset( $c['tristate_dock_inset_right_px'] ) ? (int) $c['tristate_dock_inset_right_px'] : 32, __( 'Симметрично слева/справа у FAB и панелей B/C.', 'mp-sticky-custom-cart' ), array( 'var' => $p . 'tristate-dock-inset-right', 'fmt' => 'unit', 'suffix' => 'px' ) );
		self::field_number( $opt, 'sticky_cart', 'tristate_dock_inset_bottom_px', __( 'Нижний отступ колонки над safe-area (px)', 'mp-sticky-custom-cart' ), isset( $c['tristate_dock_inset_bottom_px'] ) ? (int) $c['tristate_dock_inset_bottom_px'] : 32, '', array( 'var' => $p . 'tristate-dock-inset-bottom', 'fmt' => 'unit', 'suffix' => 'px' ) );
